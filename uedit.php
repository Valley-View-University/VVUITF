...
$sql="SELECT * from user where user_id=$_SESSION[uid]";
	...
	if (mysql_num_rows($rows)>0)
	{
		...
		echo "<tr><td>Country</td><td> :</td><td><select name='devices'>
<option value='$row[country]' selected='selected'>Ghana</option>
<option value='$row[country]'>Nigeria</option>
<option value='$row[country]'>Liberia</option>
<option value='$row[country]'>Cote D'Voire</option>
</select><span id='k' style='color: red';/></span></td></tr>";
		echo "</table>";	
	}
